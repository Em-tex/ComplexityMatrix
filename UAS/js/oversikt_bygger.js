document.addEventListener('DOMContentLoaded', async () => {
    const fieldIdToDetails = {
        ...
        'test-development': { label: 'Test and Development', section: 'fleet' },
        // Operations
        ...
        'mangler-oat-empic': { label: 'Missing data in EMPIC', section: 'performance' },
        'tid-siste-tilsyn': { label: 'Time since last audit', section: 'performance' },
        'niva1-avvik': { label: 'Level 1 finding', section: 'performance' },
    ...
    const valueToDisplayTextMap = {
        ...
        'Middels': 'Medium', 'Alvorlig': 'Severe',
        ...
        for (const [id, details] of Object.entries(fieldIdToDetails)) {
            ...
            options.forEach(([optionValue, score], index) => {
                ...
                if (typeof score === 'object' && score.type === 'additive-dependent') {
                    scoreDisplay = `${score.baseValue} + (Score for Pilots) + (Score for Bases)`;
                } else {
                    scoreDisplay = score;
                }
