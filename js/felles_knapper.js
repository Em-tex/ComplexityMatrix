...
(function() {
    ...
    let html = `
        <button id="download-csv-button">
            ...
            <span data-i18n="buttons.downloadData">Last ned data</span>
        </button>
    `;
    ...
    if (sharepointUrl) {
        html += `
            <a href="${sharepointUrl}" target="_blank" class="sharepoint-btn">
                <i class="fa-solid fa-folder" style="margin-right: 8px;"></i> <span data-i18n="buttons.sharepointFolder">Sharepoint mappe</span>
            </a>
        `;
    }

    if (listUrl) {
        const listKey = isMsat ? "buttons.profileListMsat" : "buttons.profileList";
        const listText = isMsat ? "MSAT Profiloversikt" : "Profiloversikt";
        html += `
            <a href="${listUrl}" target="_blank" class="sharepoint-btn">
                <i class="fa-solid fa-list" style="margin-right: 8px;"></i> <span data-i18n="${listKey}">${listText}</span>
            </a>
        `;
    }
    ...
    html += `
        <button id="print-pdf-button">
            ...
            <span data-i18n="buttons.printPdf">Print til PDF</span>
        </button>
        <button id="load-csv-button" class="load-button">
            ...
            <span data-i18n="buttons.loadData">Last inn data</span>
        </button>
        <button id="clear-form-button">
            ...
            <span data-i18n="buttons.resetForm">Reset skjema</span>
        </button>
