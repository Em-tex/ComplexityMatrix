document.addEventListener('DOMContentLoaded', async () => {
    const STORAGE_KEY = 'camoAssessmentData';
    let scoringRules = {};
    ...
    const fieldData = [
        { id: 'perf-hazard-identification', section: 'performance' },
        { id: 'perf-hazard-management', section: 'performance' },
        { id: 'perf-moc', section: 'performance' },
        { id: 'perf-changes-no-approval', section: 'performance' },
        { id: 'perf-changes-after-approval', section: 'performance' },
        ...
        { id: 'comp-loc-line-perm', section: 'complexity' },
        { id: 'comp-loc-base', section: 'complexity' },
        { id: 'comp-loc-line-non-perm', section: 'complexity' },
        ...
        { id: 'comp-ac-large-aeroplanes', section: 'complexity' },
        { id: 'comp-ac-large-helicopters', section: 'complexity' },
    ...
    async function init() {
        try {
            const response = await fetch('data/scoring_camo.json');
            scoringRules = await response.json();
        } catch (error) {
            console.error('Failed to load scoring rules for CAMO:', error);
    ...
    function updateGaugesAndSummaries(totals) {
        ...
        updateGauge('complexity', totals.complexity, 100); // Bruker 100 som visuell maks
        updateGauge('total', grandTotal, 150); // Bruker 150 som visuell maks
    }
